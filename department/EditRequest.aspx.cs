...
public partial class department_EditRequest : System.Web.UI.Page
{
    ...
    protected void btnConfirm_Click(object sender, EventArgs e)
    {
        ...
        Staff emp = ((Staff)Session["user"]);
        Department dept = emp.Department;
        Staff head = me.getHeadOfDepartment(dept);
        try {
            client.sendEmail(head.Email, "Requst from " + emp.Name + " On Date " + DateTime.Today.ToShortDateString(), "Please login to view detail:  http://http://10.10.1.162/LogicUniversityWebSite/store/");
        }
        catch (Exception ex)
        {
