...
public partial class store_ProcessOrder : System.Web.UI.Page
{
    public void Page_Load(object sender, EventArgs e)
    {
        ...
        else if (!((Staff)Session["user"]).Role.ToLower().Equals("supervisor"))
        {
    ...
    protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        //Adjustment_Details adjustment = null;
        //Session["adj"] = adjustment;

        switch (e.CommandName)
        {
            case "Approve":
                Adjustment_Details ad = mS.getAdjustmentDetailByID(adjustDetailId);
                mS.approveAdjustment(ad, ad.Reject_Reason);
                break;
            case "Reject":
                Adjustment_Details ad2 = mS.getAdjustmentDetailByID(adjustDetailId);
                TextBox t1 = (TextBox)this.GridView1.Rows[Index].Cells[5].FindControl("TextBox1");
                string tb = t1.Text;
                
                ad2.Reject_Reason = tb;
                ad2.Status = "Rejected";
                mS.approveAdjustment(ad2);
                break;
        }
